...
def PI_score(Pd, D, terms, y, norm_y, var = 0):
    """
    Calculate the score based on the provided data.
    ...
    # Calculate the score for each dimension
    score = (np.linalg.norm((Pd - Pd_sub)@y[:, var:var+1],axis=0) / norm_y).sum()
    y_rest = y[:, [i != var for i in range(y.shape[1])]].sum(axis = 1)
    Pdy =  (Pd_sub)@y[:, var:var+1]
    sum_y = Pdy + y_rest[:,None]
    pi_scr = 1 - np.abs(sum_y)
    pi_scr = np.array([np.maximum(0,ps) for ps in pi_scr])
    pi_scr = np.linalg.norm(pi_scr) / (10 * np.sqrt(y.shape[0]))
    # print
    score += pi_scr
    return score /2.0


def iter_scores_PI(D, y,n_terms, stepwise=False, stepsize=1, replace=False, backwards = True):
    """
    ...
    for var in range(n_vars):
        ...
        if stepwise:
            if backwards:
                ...
                while n_itens > n_terms:
                    ...
                    all_scores = Parallel(n_jobs=1)(
                        delayed(PI_score)(Pd, D, comb, y, norm_y, var)
                ...
                while n_itens > n_terms:
                    ...
                    all_scores = Parallel(n_jobs=-1)(
                        delayed(PI_score)(Pd, D, survivors + list(comb), y, norm_y, var)
                        for comb in combinations_list
                    )
            ...
            results_dict[f'var_{var}'] = best_combination, removed_order, history_values
        else:
            ...
            results_dict[f'var_{var}'] = best_combination, best_score, all_scores, combinations_list,
    return results_dict
